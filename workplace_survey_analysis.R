...
workplace_survey_answers <- read_document(file="/Users/briandsouza/Documents/MSc. in Business Analytics - Fall term/Spring term/Text analytics/A1 - Speech to text corpus presentation/david_survey_answers_team5.docx")
...
#workplace_survey_answers <- read_document(file="/Users/briandsouza/Documents/MSc. in Business Analytics - Fall term/Spring term/Text analytics/A1 - Speech to text corpus presentation/ayara_Survey Answers Team 5.docx")
...
#Tokenizing, finding frequency of tokens for each question and creating a frequency plot
data("stop_words")
...
question_1_token_freq

#freq_hist_question_1 <-question_1_token_freq %>%
#  filter(n > 3) %>% 
...
question_2_token_freq <- question_2_df %>%
  ...
  anti_join(cust_stop) %>%
  count(word, sort=TRUE)
question_2_token_freq

#freq_hist_question_2 <-question_2_token_freq %>%
#  filter(n > 3) %>% 
#  mutate(word = reorder(word,n)) %>%
#  ggplot(aes(word, n)) + geom_col() + xlab(NULL) + coord_flip()
#freq_hist_question_2
...
#freq_hist_question_3 <-question_3_token_freq %>%
#  filter(n > 3) %>% 
#  mutate(word = reorder(word,n )) %>%
#  ggplot(aes(word, n))+
#  geom_col()+
#  xlab(NULL)+
#  coord_flip()
#freq_hist_question_3

#question_3_bigram <- question_3_df %>%
#  unnest_tokens(bigram,  text, token = "ngrams", n=2)
#question_3_bigram
#question_3_bigram %>% count(bigram, sort = T)

#question_3_bigram_separated <- question_3_bigram %>%
#  separate(bigram, c("word1", "word2"), sep = " ")
#question_3_bigram_separated

#question_3_bigram_filtered <- question_3_bigram_separated %>%
#  filter(!word1 %in% c(stop_words$word, cust_stop$word)) %>%
#  filter(!word2 %in% c(stop_words$word, cust_stop$word))
#question_3_bigram_filtered

#question_3_bigram_united_freq <- question_3_bigram_filtered %>%
#  unite(bigram, word1, word2, sep=" ") %>%
#  count(bigram, sort = T)
#question_3_bigram_united_freq

#freq_hist_question_3_bigram <- question_3_bigram_united_freq %>%
#  mutate(bigram = reorder(bigram,n)) %>%
#  ggplot(aes(bigram, n, fill = n)) + geom_col() + xlab(NULL) + coord_flip() + theme_survey()
#freq_hist_question_3_bigram

#QUADROGRAM
question_3_quadrogram <- question_3_df %>%
  unnest_tokens(quadrogram,  text, token = "ngrams", n=4)
question_3_quadrogram
question_3_quadrogram_freq <- question_3_quadrogram %>% count(quadrogram, sort = T)
question_3_quadrogram_freq
...
#freq_hist_question_4 <-question_4_token_freq %>%
#  filter(n > 4) %>% 
#  mutate(word = reorder(word,n )) %>%
#  ggplot(aes(word, n))+
#  geom_col()+
...
question_5_token_freq <- question_5_df %>%
  ...
  count(word, sort=TRUE)
question_5_token_freq

freq_hist_question_5 <-question_5_token_freq %>%
  filter(n > 1) %>% 
  mutate(word = reorder(word,n )) %>%
  ggplot(aes(word, n)) + geom_col(fill = "steelblue") + xlab(NULL) + theme_survey()
freq_hist_question_5

#TF-IDF

survey_df_tf_idf <- bind_rows(
  mutate(question_1_token_freq, question = "First"),
  mutate(question_2_token_freq, question = "Second"),
  mutate(question_3_token_freq, question = "Third"),
  mutate(question_4_token_freq, question = "Fourth"),
  mutate(question_5_token_freq, question = "Fifth")
)
survey_df_tf_idf

survey_df_tf_idf_1 <- survey_df_tf_idf %>% bind_tf_idf(word, question, n)
...
#question_1_token_freq %>% with(wordcloud(word, n, max.words = 100))
#question_2_token_freq %>% with(wordcloud(word, n, max.words = 100))
#question_3_token_freq %>% with(wordcloud(word, n, max.words = 100))
#question_4_token_freq %>% with(wordcloud(word, n, max.words = 100))
...
#######################################################################################################################################
#Predictive analysis - Unsupervised
